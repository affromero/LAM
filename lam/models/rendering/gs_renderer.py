...
from lam.models.rendering.utils.utils import MLP, trunc_exp
...
class Camera:
    ...
    def from_c2w(c2w, intrinsic, height, width):
        ...
        FoVx, FoVy = intrinsic_to_fov(
            intrinsic, w=torch.tensor(width, device=w2c.device), h=torch.tensor(height, device=w2c.device)
        )
    ...
    def __init__(
        ...
        for key, out_ch in self.attr_dict.items():
            ...
            if not (key == "shs" and use_rgb):
                if (key == "opacity" and self.fix_opacity) or (key == "rotation" and self.fix_rotation):
                    pass
                else:
...
class DecoderCrossAttn(nn.Module):
    def __init__(self, query_dim, context_dim, num_heads, mlp=False, decode_with_extra_info=None):
        ...
        self.cross_attn = CrossAttnBlock(
            inner_dim=query_dim, cond_dim=context_dim, num_heads=num_heads, feedforward=mlp, eps=1e-5
        )
        ...
        if decode_with_extra_info is not None:
            if decode_with_extra_info["type"] == "dinov2p14_feat":
                ...
                self.cross_attn_color = CrossAttnBlock(
                    inner_dim=query_dim, cond_dim=context_dim, num_heads=num_heads, feedforward=False, eps=1e-5
                )
                ...
                self.cross_attn_color = CrossAttnBlock(
                    inner_dim=query_dim, cond_dim=384, num_heads=num_heads, feedforward=False, eps=1e-5
                )
                ...
                self.cross_attn_color = CrossAttnBlock(
                    inner_dim=query_dim, cond_dim=64, num_heads=num_heads, feedforward=False, eps=1e-5
                )
    ...
    def forward(self, pcl_query, pcl_latent, extra_info=None):
        ...
        if self.decode_with_extra_info is not None:
            ...
            if self.decode_with_extra_info["type"] == "dinov2p14_feat":
                ...
                out_dict["fine"] = out
                return out_dict
            if self.decode_with_extra_info["type"] == "decoder_dinov2p14_feat":
                img_feat = self.encoder(extra_info["image"])
                out = self.cross_attn_color(out, img_feat)
                out_dict["fine"] = out
                return out_dict
            if self.decode_with_extra_info["type"] == "decoder_resnet18_feat":
...
class GS3DRenderer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.gs_net = GSLayer(
            ...
            use_fine_feat=True
            if decode_with_extra_info is not None and decode_with_extra_info["type"] is not None
            else False,
        )
    ...
    def query_latent_feat(
        self, positions: Float[Tensor, "*B N1 3"], flame_data, latent_feat: Float[Tensor, "*B N2 C"], extra_info
    ):
        ...
        **kwargs,
        ...
        for b in range(batch_size):
            gs_model = self.gs_refine_net(
                query_gs_features[b], query_points[b], x_fine=None, gs_raw_attr=gs_raw_attr_list[b]
            )
            gs_model_list.append(gs_model)
        return gs_model_list, query_points, flame_data, query_gs_features

    def forward_animate_gs(
        self, gs_model_list, query_points, flame_data, c2w, intrinsic, height, width, background_color, debug=False
    ):
        ...
        for b in range(batch_size):
            ...
            animatable_gs_model_list: list[GaussianModel] = self.animate_gs_model(
                gs_model, query_pt, self.get_sing_batch_smpl_data(flame_data, b), debug=debug
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        out = self.forward_animate_gs(
            gs_model_list, query_points, flame_data, c2w, intrinsic, height, width, background_color, debug
        )
    ...
    for k, v in out.items():
        ...
        for b_idx in range(len(v)):
            ...
            for v_idx in range(v.shape[1]):
                ...
                if "normal" in k:
                    img = ((v[b_idx, v_idx].permute(1, 2, 0).detach().cpu().numpy() + 1.0) / 2.0 * 255).astype(np.uint8)
                else:
                ...
                if "mask" in k:
                    ...
                    blend_img = (
                        (render_images[v_idx].transpose(1, 2, 0) * 255 * 0.5 + np.tile(img, (1, 1, 3)) * 0.5)
                        .clip(0, 255)
                        .astype(np.uint8)
                    )
                    cv2.imwrite(
                        save_path,
                        np.hstack([np.tile(img, (1, 1, 3)), render_img.astype(np.uint8), blend_img])[:, :, (2, 1, 0)],
                    )
                else:
                    print(save_path, k)
