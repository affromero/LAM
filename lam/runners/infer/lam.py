...
class LAMInferrer(Inferrer):
    ...
    def _default_source_camera(
        self, dist_to_center: float = 2.0, batch_size: int = 1, device: torch.device = torch.device("cpu")
    ):
        # return: (N, D_cam_raw)
    ...
    def infer_video(
        ...
        render_bg_colors = (
            torch.ones(N, render_cameras.shape[1], 1, device=self.device, dtype=torch.float32) * 0.0
        )  # 1.
    ...
    def infer_single(
        ...
        if ref_bg < 1.0:
            if "VFHQ_TEST" in image_path:
                mask_path = (
                    image_path.replace("/VFHQ_TEST/", "/mask/").replace("/images/", "/mask/").replace(".png", ".jpg")
                )
            else:
                mask_path = (
                    image_path.replace("/vfhq_test_nooffset_export/", "/mask/")
                    .replace("/images/", "/mask/")
                    .replace(".png", ".jpg")
                )
        ...
        if vis_motion:
            ...
            vis_ref_img = np.tile(
                cv2.resize(vis_ref_img, (rgb[0].shape[1], rgb[0].shape[0]), interpolation=cv2.INTER_AREA)[
                    None, :, :, :
                ],
                (rgb.shape[0], 1, 1, 1),
            )
        ...
        if save_img and dump_image_dir is not None:
            ...
            save_color_points(
                res["cano_gs_lst"][0].xyz, res["cano_gs_lst"][0].shs[:, 0, :], "framework_img.obj", sv_fd=dump_cano_dir
            )
            ...
            from lam.models.rendering.utils import mesh_utils
    ...
    def infer(self):
        ...
        for image_path in tqdm(image_paths, disable=not self.accelerator.is_local_main_process):
            try:
                ...
                subdir_path = subdir_path.removeprefix("/")
                # hard code
                ...
                dump_image_dir = os.path.join(self.cfg.image_dump, subdir_path, f"{uid}")
